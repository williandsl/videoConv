...
import org.example.client.downloader.YoutubeProgressCallback;
...
import static org.example.client.model.Utils.closeSilently;
...
public class DownloaderImpl implements Downloader {
    ...
    private String download(RequestWebpage request) throws IOException, YoutubeException {
        ...
        org.example.client.downloader.YoutubeCallback<String> callback = request.getCallback();
    ...
    private File download(RequestVideoFileDownload request) throws IOException {
        ...
        org.example.client.downloader.YoutubeCallback<File> callback = request.getCallback();
    ...
    private Void download(RequestVideoStreamDownload request) throws IOException {
        Format format = request.getFormat();
        org.example.client.downloader.YoutubeCallback<Void> callback = request.getCallback();
    ...
    private void download(Request<?, ?> request, Format format, OutputStream os) throws IOException {
        Map<String, String> headers = request.getHeaders();
        org.example.client.downloader.YoutubeCallback<?> callback = request.getCallback();
        int maxRetries = request.getMaxRetries() != null ? request.getMaxRetries() : config.getMaxRetries();
        Proxy proxy = request.getProxy();
    ...
    private void downloadStraight(Format format, OutputStream os, Map<String, String> headers, Proxy proxy, org.example.client.downloader.YoutubeCallback<?> callback) throws IOException {
    ...
    private void downloadByPart(Format format, OutputStream os, Map<String, String> headers, Proxy proxy, org.example.client.downloader.YoutubeCallback<?> listener) throws IOException {
        long done = 0;
        int partNumber = 0;
        ...
        try {
            ...
            while ((read = is.read(buffer)) != -1) {
                ...
                if (progress > lastProgress) {
                    if (listener instanceof org.example.client.downloader.YoutubeProgressCallback) {
